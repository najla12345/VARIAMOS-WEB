let setup_elements = function setup_elements(graph, elements, custom_attributes,
  c_clon_cells, c_constraints_ic, toolbar, c_type) {
  if (elements == null) {
    ...
    for (let i = 0; i < elements.length; i++) {
      addVertex(graph, toolbar, elements[i].src, elements[i].wd, elements[i].hg,
        elements[i].style, elements[i].type, elements[i].pname, 
        custom_attributes, c_clon_cells, c_constraints_ic);
    }
  }

  set_tbcontainer_style();

  // This makes makes the container better.
  function set_tbcontainer_style() {
    let tbContainer = document.getElementById('tbContainer');
    tbContainer.style.display = 'flex';
    tbContainer.style.flexWrap = 'wrap';
    tbContainer.style.justifyContent = 'space-evenly';
    tbContainer.style.alignItems = 'flex-end';
    tbContainer.style.alignContent = 'space-evenly';

  }

  function addVertex(graph, toolbar, icon, w, h, style, type, namepalette,
    custom_attributes, c_clon_cells, c_constraints_ic) {
      ...
      for (let z = 0; z < custom_attributes.length; z++) {
        if ((custom_attributes[z]["types"].indexOf(type) > -1)) {
          for (let j = 0; j < custom_attributes[z]["custom_attributes"].length; j++) {
            node.setAttribute(custom_attributes[z]["custom_attributes"][j]["name"], 
              custom_attributes[z]["custom_attributes"][j]["def_value"]);
          }
        }
      }
    }
      ...
      if (oncreation_allowed) {
        ...
        new_cells.forEach(element => {
          ...
          if (["goal", "quality", "task", "resource", "secconstraint"].includes(type)) {
            if (cell === null || (cell !== null && cell.getAttribute('type') 
              === 'organization')) {
